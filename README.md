...
* v1.6
  * 修改密码记录方式, 新增特殊符号记录方式.
  * 增加屏幕随时截图功能, 在托盘图标处右键菜单中启动相关组件.

* v1.5
  * 增加新功能，现在可以使用右键-move来固定时钟的默认位置。
  * 增加新接口，使用Clock.ParaWriter.updatePara(HashMap<String, String> paras)可以更新配置文件。
  * 发现新bug：当分屏且各个屏幕缩放比例不同时，右键菜单会出现显示错误。重启程序可解决。
